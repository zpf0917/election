...
contract("Election", function(accounts) { // Inject all the accounts existing in our testing environment
    ...
    it("allows a voter to cast a vote", function() {
        return Election.deployed().then(function(instance){
            electionInstance = instance;
            candidateId = 1;
            return electionInstance.vote(candidateId, {from: accounts[0]}); // why not need web3.eth
    ...
    it("throws an exception for invalid candidates", function() {
        ...
        }).then(assert.fail).catch(function(error) { // catch the error and insert it into the callback function
    ...
    it("throws an exception for double voting", function() {
        return Election.deployed().then(function(instance) {
            ...
            var voteCount = candidate2[2];
            assert.equal(voteCount, 1, "candidate 2 accepts the first vote");
